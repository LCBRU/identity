...
class TestScheduling:
    ...
    def fixtures(self, client, faker, mock_extract_data, mock_produce_demographics_result, mock_extract_pre_pmi_details, mock_log_exception):
        self.client = client
        self.faker = faker
    ...
    def test__schedule_lookup_tasks__request_not_found(self):
        do_lookup_tasks(1)
        self.assert_not_called(called=['mock_log_exception'])
    ...
    def test__schedule_lookup_tasks__submitted(self):
        dr = self.test_helper.get_demographics_request__submitted()

        do_lookup_tasks(dr.id)

        self.assert_not_called(called=['mock_extract_data'])

    def test__schedule_lookup_tasks__extracted(self):
        dr = self.test_helper.get_demographics_request__request_data_extracted()

        do_lookup_tasks(dr.id)

        self.assert_not_called(called=['mock_extract_pre_pmi_details'])
