...
from identity.redcap_import.model import BriccsParticipantImportStrategy
...
def import_new_participants():
    ...
    for r in RedcapInstance.query.all()[0:1]:
        strategy = BriccsParticipantImportStrategy()
...
def _load_instance_participants(instance):
    with redcap_engine(instance.database_name) as conn:
        current_app.logger.info(f'importing participants from REDCap instance: {r.name}')
