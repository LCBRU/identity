...
def upgrade(migrate_engine):
    ...
    bidp = Table("bioresource_id_provider", meta, autoload=True)
    lidp = Table("legacy_id_provider", meta, autoload=True)
    pridp = Table("pseudo_random_id_provider", meta, autoload=True)
    sidp = Table("sequential_id_provider", meta, autoload=True)
    ...
    for type in ['bioresource_id_provider', 'legacy_id_provider', 'pseudo_random_id_provider', 'sequential_id_provider']:
        ...
        inp.append_constraint(ForeignKeyConstraint([id_provider_id], [idp.c.id]))
